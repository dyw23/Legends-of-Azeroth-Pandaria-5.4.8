...
CinematicSequenceMgr* CinematicSequenceMgr::instance()
{
...
size_t CinematicSequenceMgr::Load()
{
    ...
    for (uint32 l_SequenceID = 0; l_SequenceID < sCinematicSequencesStore.GetNumRows(); ++l_SequenceID)
    {
        ...
        std::string l_ModelFileName = l_CameraEntry->filename[sObjectMgr->GetDBCLocaleIndex()];
        ...
        if (!l_ModelFile)
        {
            TC_LOG_INFO("server.loading", "Cinematic camera '%s': does not exist!", l_ModelFileName.c_str());
            continue;
        }
        ...
        {
            TC_LOG_INFO("server.loading", "Cinematic camera '%s': invalid magic header!", l_ModelFileName.c_str());
            continue;
        }
        ...
        if (!l_Header->CameraCount || !l_Header->CameraOffset)
        {
            ...
            TC_LOG_INFO("server.loading", "Cinematic camera '%s': no cameras!", l_ModelFileName.c_str());
            continue;
        }
        ...
        {
            TC_LOG_INFO("server.loading", "Cinematic camera '%s': no translations!", l_ModelFileName.c_str());
            free(l_FileBuffer);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u Cinematic cameras in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
CinematicSequence const* CinematicSequenceMgr::GetSequence(uint32 p_SequenceID)
{
    ...
    return (l_It != m_Sequences.end()) ? l_It->second : nullptr;
}
...
void CinematicSequence::GetPositionAtTime(uint32 p_Time, float* p_X, float* p_Y, float* p_Z) const
{
    ...
    *p_Z = 0;
    ...
    {
        CinematicKeyFrame* l_NextKeyFrame = (l_KeyFrame + 1 != KeyFrames + KeyFramesCount) ? l_KeyFrame + 1 : nullptr;
